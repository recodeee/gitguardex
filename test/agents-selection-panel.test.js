...
test('renderAgentSelectionPanel shows a dmux-style GitGuardex shell', () => {
  ...
  assert.match(output, /Project focus/);
...
test('interactive panel keys move focus, toggle agents, and adjust codex accounts', () => {
  ...
  const projectHelp = applyAgentSelectionKey(state, 'p');
  assert.equal(projectHelp.action, 'render');
  assert.match(projectHelp.state.message, /Project panes are managed in gx cockpit/);
  const paneMenuMessage = applyAgentSelectionKey(state, 'm');
  assert.equal(paneMenuMessage.action, 'render');
  assert.match(paneMenuMessage.state.message, /m or Alt\+Shift\+M/);
  ...
  const paneMenuObjectHelp = applyAgentSelectionKey(state, { name: 'm', alt: true, shift: true });
  assert.equal(paneMenuObjectHelp.action, 'render');
  assert.match(paneMenuObjectHelp.state.message, /selected pane/);
  const closeHelp = applyAgentSelectionKey(state, 'x');
  assert.equal(closeHelp.action, 'render');
  assert.match(closeHelp.state.message, /Close is available/);
  const projectFocusHelp = applyAgentSelectionKey(state, 'P');
  assert.equal(projectFocusHelp.action, 'render');
  assert.match(projectFocusHelp.state.message, /Project focus is available/);
  const reopenHelp = applyAgentSelectionKey(state, 'r');
  assert.equal(reopenHelp.action, 'render');
  assert.match(reopenHelp.state.message, /pane menu/);
