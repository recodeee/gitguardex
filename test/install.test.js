...
test('codex-agent launches codex inside a fresh sandbox worktree and auto-prunes clean branches on exit', () => {
  ...
  const setupResult = runNode(['setup', '--target', repoDir, '--no-global-install'], repoDir);
  assert.equal(setupResult.status, 0, setupResult.stderr || setupResult.stdout);
  let setupCommit = runCmd('git', ['add', '.'], repoDir);
  assert.equal(setupCommit.status, 0, setupCommit.stderr);
  setupCommit = runCmd('git', ['commit', '-m', 'apply gx setup'], repoDir, {
    ALLOW_COMMIT_ON_PROTECTED_BRANCH: '1',
  });
  assert.equal(setupCommit.status, 0, setupCommit.stderr || setupCommit.stdout);
  ...
  assert.equal(fs.existsSync(launchedCwd), false, 'clean sandbox should be auto-pruned after codex exit');
...
test('codex-agent supports --codex-bin override before positional arguments', () => {
  ...
  const launchedArgs = fs.readFileSync(argsMarker, 'utf8').trim();
  assert.match(launchedArgs, /--model gpt-5\.4-mini/);
  assert.equal(fs.existsSync(launchedCwd), false, 'clean override run should auto-prune sandbox');
});
