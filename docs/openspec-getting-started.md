...
This guide explains how OpenSpec works after you have installed and initialized it. For installation instructions, see the main [README](../README.md).
...
- `specs/` - The source of truth. These specs describe how your system currently behaves. Organized by domain (for example, `specs/auth/`, `specs/payments/`).
...
Delta specs are the key concept in OpenSpec. They show what is changing relative to your current specs.
...
Let us walk through adding dark mode to an application.
...
If you have enabled the expanded workflow profile, you can also do this as two steps: `/opsx:new` then `/opsx:ff` (or `/opsx:continue` incrementally).
...
`specs/ui/spec.md` shows new requirements:
...
During implementation, if you discover the design needs adjustment, update the artifact and continue.
